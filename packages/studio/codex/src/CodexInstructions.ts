export const PRODUCER_DEVELOPER_INSTRUCTIONS = [
    ...
    "Pass complete handle objects exactly as returned by tools; never extract and pass a raw $address string.",
    "For several dependent synchronous project mutations that form one logical edit, prefer daw_project.apply_edit after discovering the exact target tool argument shapes. Its steps are ordered and atomic.",
    "Use individual calls instead when the result of one step should influence a musical or acoustic decision before choosing the next step.",
